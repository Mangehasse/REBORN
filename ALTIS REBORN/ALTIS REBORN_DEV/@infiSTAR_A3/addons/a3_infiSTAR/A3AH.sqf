...
/* *********************************www.infiSTAR.de********************************* */
diag_log '<infiSTAR.de> VERSION: 28-Sep-2016 21-05-38 infiSTAR.de AHAT (v0241)';
comment 'Antihack & AdminTools - Christian Lorenzen - www.infiSTAR.de - License: (CC)';
UPDATEEMAIL='magnus@jetprint.se';
INFISTARVERSION='28-Sep-2016 21-05-38 - v0241';
...
if(!_return)exitWith
{
	diag_log format['<infiSTAR.de> %1 - serverCommandPassword NOT SET!    magnus@jetprint.se - 28-Sep-2016 21-05-38 - v0241 - %2 - %3',_t,serverName,productVersion];
...
{
	diag_log format['<infiSTAR.de> %1 - infiSTAR_AdminMenu.hpp in your MPmission is NOT UPDATED    magnus@jetprint.se - 28-Sep-2016 21-05-38 - v0241 - %2 - %3',_t,serverName,productVersion];
...
{
	_log = format['<infiSTAR.de> %1 - call.fsm missing in your MPmission!    magnus@jetprint.se - 28-Sep-2016 21-05-38 - v0241 - %2 - %3',_t,serverName,productVersion];
...
FNC_A3_LOG_RAND_VAR = compileFinal "
	'ARMA_LOG' callExtension format['A3_RANDOMVAR_LOG:%1 (v0241)',_this];
	diag_log ('<infiSTAR.de>RandomVarLog| '+_this+' (v0241)');
";
...
_fnc_RandomGen =
{
	_fnc_actualGen = {
		...
		_arr = ['K','J','K','D','3','e','m','o','A','S','p','s','e','M','L','8','5','M','c','5','d','D','s','F','0','6','c','s','M','6'];
...
_FNC_AH3458_KICKLOG = call _fnc_RandomGen;format['_FNC_AH3458_KICKLOG: %1',_FNC_AH3458_KICKLOG] call FNC_A3_LOG_RAND_VAR;
_FNC_AH3458_KICKLOGSPAWN = call _fnc_RandomGen;format['_FNC_AH3458_KICKLOGSPAWN: %1',_FNC_AH3458_KICKLOGSPAWN] call FNC_A3_LOG_RAND_VAR;
...
_AH3458KICKLOG = call _fnc_RandomGen;format['_AH3458KICKLOG: %1',_AH3458KICKLOG] call FNC_A3_LOG_RAND_VAR;
...
if(_useTildMenu)then{_cMenu pushBack ('#USER:USERMENU'+_customcommandingMenu);};
_fnc_RandomGen =
{
...
_AH3458_KICKLOG = call _fnc_RandomGen;format['_AH3458_KICKLOG: %1',_AH3458_KICKLOG] call FNC_A3_LOG_RAND_VAR;
...
FNC_A3_HACKLOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_HACKLOG:%1 [FPS: %2|THREADS: %3] (v0241)',_this,diag_fps,count diag_activeSQFScripts];
	diag_log ('<infiSTAR.de>HL| '+_this+' (v0241)');
";
FNC_A3_SURVEILLANCELOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_SURVEILLANCELOG:%1 [FPS: %2|THREADS: %3] (v0241)',_this,diag_fps,count diag_activeSQFScripts];
	diag_log ('<infiSTAR.de>SL| '+_this+' (v0241)');
";
FNC_A3_ADMINLOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_ADMINLOG:%1 (v0241)',_this];
	diag_log ('<infiSTAR.de>AdminLog| '+_this+' (v0241)');
";
FNC_A3_CONNECTLOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_CONNECTLOG:%1 (v0241)',_this];
	diag_log ('<infiSTAR.de>ConnectLog| '+_this+' (v0241)');
";
FNC_A3_TOKENLOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_TOKENLOG:%1 (v0241)',_this];
	diag_log ('<infiSTAR.de>TokenLog| '+_this+' (v0241)');
";
FNC_A3_RESOLVESTEAMNAMELOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_RESOLVESTEAMNAMELOG:%1 (v0241)',_this];
	diag_log ('<infiSTAR.de>ResolveSteamName| '+_this+' (v0241)');
...
_A3AHstring = "
	...
	FNC_INFISERVERKICK = {
		...
		diag_log format['<infiSTAR.de>KickLog| %1(%2)   (v0241)',_name,_uid];
	};
	...
	FNC_A3_infiSTARBAN = {
		...
		if(_reason == '')then
		{
			'ARMA_LOG' callExtension format['A3_BANLOG:%1(%2) (v0241)',_name,_uid];
			diag_log format['<infiSTAR.de>BanLog| %1(%2)   (v0241)',_name,_uid];
		}
		else
		{
			'ARMA_LOG' callExtension format['A3_BANLOG:%1(%2) %3 (v0241)',_name,_uid,_reason];
			diag_log format['<infiSTAR.de>BanLog| %1(%2) %3 (v0241)',_name,_uid,_reason];
		};
...
"; if(_testserver)then{ _A3AHstring = _A3AHstring + "
	...
	[_name,_uid,_owner,_packet1] spawn {
		...
		{
			[_name,_uid,_owner,if(_res in ['','1'])then{'BLACKLISTED'}else{format['BLACKLISTED: %1',_res]}] call FNC_A3_infiSTARBAN;
		};
	};
"; }else{ _A3AHstring = _A3AHstring + "
	...
	[
		...
		'76561198196608571','76561198018421000','76561198117300789',
		...
		'76561198166992322','76561198063808946','76561198116706040','76561198043920423',
		'76561198254277744','76561197982194585','76561198152271408',
		...
		'76561198197873415','76561197975205130','76561198079432235','76561198166830499',
		'76561198202742576','76561198105830708',
		'0'
	];
	if(_uid in _blackListed__Hacker_or_Vac_Ban)exitWith
	{
		[_name,_uid,'BLACKLISTED'] call FNC_A3_infiSTARTMPBAN;
	};
"; }; _A3AHstring = _A3AHstring + "
	if({((toLower _name) find _x != -1)} count ['thirtysix','adolf','le hippo'] > 0)exitWith{
		_log = format['BadName: %1 (KICKED)',_name];
		[_name,_uid,'SLOG_SKICK',toArray(_log)] call "+_FNC_AH3458_KICKLOG+";
	};
...
fnc_infiSTAR_PlayerLog = compileFinal ([_fnc_infiSTAR_PlayerLog] call fnc_CompilableString);
							...
							"; if(_UVC)then{ _A3AHstring = _A3AHstring + "
								if((!("+str _UVW+") && {_vehstype in "+str _ForbiddenVehicles+"}) || (("+str _UVW+") && {!(_vehstype in "+str _VehicleWhiteList+")}))then
								{
									...
									if(_crew isEqualTo [])then
									{
										...
										['SERVER','SERVER','HLOG',toArray(_log)] call "+_FNC_AH3458_KICKLOG+";
									...
									else
									{
										...
										_log = format['BadVehicle (S-UVC+LVC): %1',_vehstype];
										{
											_name = name _x;
											_puid = getPlayerUID _x;
											[_name,_puid,'HLOG',toArray(_log)] call "+_FNC_AH3458_KICKLOG+";
										} forEach _crew;
										_veh call fnc_deleteObject;
									};
								};
						...
						if(!isNull _veh)then
						{
							if((_plrObj == _veh) && (_vehstype != _vehptype))then
							{
								...
								if!(_veh in _vehicles)then
								{
									...
									[_name,_puid,'SLOG',toArray(_log)] call "+_FNC_AH3458_KICKLOG+";
								};
							};
						};
					};
	...
	"+_FNC_AH3458_KICKLOG+" = compileFinal ([_FNC_AH3458_KICKLOG] call fnc_CompilableString);
	...
	_FNC_PVAH_AdminReq = {
		...
		if(isNil '_clientNetID')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq clientNetID is Nil (v0241)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _clientNetID != 'STRING')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq clientNetID wrong typeName: %1   %2 (v0241)',typeName _clientNetID,_clientNetID];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(_clientNetID == '')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq clientNetID is empty (v0241)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		_ObjFromNetID = objectFromNetId _clientNetID;
		if(isNil '_ObjFromNetID')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq playerObj is Nil (v0241)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _ObjFromNetID != 'OBJECT')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq ObjFromNetID wrong typeName: %1   %2 (v0241)',typeName _ObjFromNetID,_ObjFromNetID];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(isNull _ObjFromNetID)exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq ObjFromNetID is Null (v0241)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_array')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq is Nil (v0241)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _array != 'ARRAY')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq wrong typeName: %1   %2 (v0241)',typeName _array,_array];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_option')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq option is Nil (v0241)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _option != 'SCALAR')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq option wrong typeName: %1   %2 (v0241)',typeName _option,_option];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(typeName _playerObj != 'OBJECT')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq playerObj wrong typeName: %1   %2 (v0241)',typeName _playerObj,_playerObj];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(isNull _playerObj)exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq playerObj is Null (v0241)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if!(_ObjFromNetID isEqualTo _playerObj)then
		{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq playerObj != _ObjFromNetID - %1/2  (v0241)',_playerObj,_ObjFromNetID];
			_log call FNC_A3_SURVEILLANCELOG;
		...
		if(!isPlayer _playerObj)then
		{
			if(_option == 1234)then
			{
				...
				if(isNil '_puid')exitWith{
					_mytime = call fnc_getserverTime;
					_log = _mytime + 'AdminReq puid is Nil (v0241)';
					_log call FNC_A3_SURVEILLANCELOG;
				};
				if(typeName _puid != 'STRING')exitWith{
					_mytime = call fnc_getserverTime;
					_log = _mytime + format['AdminReq puid wrong typeName: %1   %2 (v0241)',typeName _puid,_puid];
					_log call FNC_A3_SURVEILLANCELOG;
				};
				
				_mytime = call fnc_getserverTime;
				_log = format['AdminReq playerObj is not a player   %1(%2 - %3) - trying to resolve player via allPlayers (v0241)',name _playerObj,getPlayerUID _playerObj,_puid];
				_log = _mytime + _log;
				_log call FNC_A3_SURVEILLANCELOG;
				if(_puid == '')exitWith{};
				{
		...
		if(!isPlayer _playerObj)exitWith{
			_mytime = call fnc_getserverTime;
			_log = format['AdminReq playerObj is not a player   %1(%2) (v0241)',name _playerObj,getPlayerUID _playerObj];
			_log = _mytime + _log;
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_tokenreceived')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq tokenreceived is Nil (v0241)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _tokenreceived != 'STRING')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq tokenreceived wrong typeName: %1   %2 (v0241)',typeName _tokenreceived,_tokenreceived];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(_tokenreceived == '')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq tokenreceived is empty (v0241)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(_clientUID isEqualTo '')then
		{
			...
			if(_foundtokenid > 0)then
			{
				...
				if(!isNil '_puidfound')then
				{
					...
					_mytime = call fnc_getserverTime;
					_log = _mytime + format['%1(%2) | AdminReq - SERVER RESOLVED UID! (v0241)',_clientName,_clientUID];
					_log call FNC_A3_SURVEILLANCELOG;
				};
			};
		};
		...
		if(!_result)exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['%1(%2) | AdminReq - BadToken %3 (v0241)',_clientName,_clientUID,_tokenreceived];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		_fnc_NoUID = {
			...
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['%1(%2) | AdminReq - !!!!!!ADMIN-LOGIN-ERROR!!!!!! (v0241)',_clientName,_clientUID];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		call {
		...
		{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['%1(%2) | AdminReq - Attempted to Use the AdminMenu (v0241)',_clientName,_clientUID];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(_option == 1234)exitWith
		{
			...
			if(_puid == _clientUID)then
			{
				if!(_clientUID in _locDevs)then{
					diag_log format['<infiSTAR.de> ******ADMIN-LOGIN******: %1(%2) (v0241)',_clientName,_clientUID];
				};
				"; if(_enableIngameLogs)then{ _A3AHstring = _A3AHstring + "
				...
				[
					'',
					{
						...
						"; if(_LogAdminActions)then{ _A3AHstring = _A3AHstring + "
							if(isNil 'fnc_adminLog')then{if(_puid in "+str _devs+")then{fnc_adminLog = compileFinal '';}else{fnc_adminLog = compileFinal '[profileName,getPlayerUID player,''ALOG'',toArray _this] call "+_AH3458KICKLOG+";';};};
						"; }else{ _A3AHstring = _A3AHstring + "
		...
		if(_option == -668)exitWith
		{
			...
			_adminObjects = [];{if((getPlayerUID _x) in _allPlayers)then{_adminObjects pushBack _x;};} forEach playableUnits;
			if(_clearLog == 0)then
			{
				{
					if(!isNull _x)then
		...
		if(_option == -667)exitWith
		{
			...
			{if(getPlayerUID _x in _allPlayers)then{(owner _x) publicVariableClient '"+_TMPBAN+"';};} forEach playableUnits;
		};
	...
	_FNC_AH3458_KICKLOGSPAWN =
	{
		...
		if(isNil '_input')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'FNC_AH3458_KICKLOGSPAWN _this is Nil (v0241)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		diag_log format['<infiSTAR.de FNC_AH3458_KICKLOGSPAWN> %1',_this];
		
		_tokenreceived = _this select 0;
		if(isNil '_tokenreceived')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + '_tokenreceived _this is Nil (v0241)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _tokenreceived != 'STRING')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['_tokenreceived wrong type %1 (v0241)',typeName _tokenreceived];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_arraysent')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + '_arraysent _this is Nil (v0241)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _arraysent != 'ARRAY')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['_arraysent wrong type %1 (v0241)',typeName _arraysent];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(typeName _netId != 'STRING')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['_netId wrong type %1 (v0241)',typeName _netId];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		diag_log format['<infiSTAR.de FNC_AH3458_KICKLOGSPAWN> %1(%2) Token %3 - objByNetID %4',_name,_puid,_tokenreceived,_objectFromNetId];
		...
		else
		{
			if(_objectUID isEqualTo _puid)then
			{
				...
				{
					diag_log format['<infiSTAR.de FNC_AH3458_KICKLOGSPAWN> %1(%2) PlayerObjectName is not equal to PlayerName!',_name,_puid,_objectName];
					_name = _objectName;
				};
			};
		};
		...
		if(!_result)exitWith
		{
			...
			diag_log format['<infiSTAR.de FNC_AH3458_KICKLOGSPAWN> %1',_log];
			[_name,_puid,'SLOG_SKICK',toArray(_log)] call "+_FNC_AH3458_KICKLOG+";
		};
		
		
		_arraysent set [0,_objectName];
		_arraysent set [1,_puid];
		_arraysent spawn "+_FNC_AH3458_KICKLOG+";
	};
	"+_FNC_AH3458_KICKLOGSPAWN+" = compileFinal ([_FNC_AH3458_KICKLOGSPAWN] call fnc_CompilableString);
	...
	_fnc_mpPackets = {
		...
		else
		{
			if(_bannable)then
			{
				...
				[_name,_puid,'BAN',toArray(_log)] call "+_FNC_AH3458_KICKLOG+";
			}
			else
			{
				_log = format['BAD_MP_packet: %1 in %2 :: %3   (S-KICK)',_verybadstringfound,_foundBad,_stringcode];
				[_name,_puid,'HLOG_SKICK',toArray(_log)] call "+_FNC_AH3458_KICKLOG+";
				[_name,_puid] call FNC_INFISERVERKICK;
			};
		};
	};
	...
	addMissionEventHandler['HandleDisconnect',{
		...
		if(alive _plyr)then
		{
			...
			if(_inCombat == 1)then
			{
				...
				'ARMA_LOG' callExtension format['A3_COMBATLOG:%1 (v0241)',_log];
				diag_log ('<infiSTAR.de>CombatLog| '+_log+' (v0241)');
...
_AH_MAIN_BLOCK = {
	...
	if(isNil'"+_AH3458KICKLOG+"')exitWith{
	...
	if!(_puid isEqualTo _clientUID)exitWith
	{
		...
		[_name,_puid,'SLOG_SKICK',toArray(_log)] call "+_AH3458KICKLOG+";
		(findDisplay 46)closeDisplay 0;
	};
	...
	if!(_puid in _admins)then
	{
		if(str _oldUIDs != '[]')then
		{
			...
			if(_announce)then
			{
				...
				[profileName,_puid,'TMPBAN',toArray(_log)] call "+_AH3458KICKLOG+";
				[] spawn "+_AHKickOFF+";
			};
		};
		{
			if(!isNil _x)then
			{
				...
				[profileName,_puid,'SLOG_SKICK',toArray(_log)] call "+_AH3458KICKLOG+";
				(findDisplay 46)closeDisplay 0;
			};
		} forEach
		[
		...
		_badclassesban = ['life_server','devcon','loki_lost_key','pooploop','rscrazzler','rsclistboxvg','vgdialog'];
		_badclasseskick = ['a3_m3editor','extdb2','infistar_lifemods','infistar_a3'];
		_cfgPatches = 'true' configClasses (configFile >> 'CfgPatches');
		{
			...
			if(_lconfigName in _badclassesban)then
			{
				...
				[_name,_puid,'BAN',toArray(_log)] call "+_AH3458KICKLOG+";
				[] spawn "+_AHKickOFF+";
			};
			if(_lconfigName in _badclasseskick)then
			{
				_log = format['Bad Addon found in CfgPatches:: %1',_configName];
				[_name,_puid,'HLOG_SKICK',toArray(_log)] call "+_AH3458KICKLOG+";
				[] spawn "+_AHKickOFF+";
			};
		} forEach _cfgPatches;
	...
	[_name,_puid,_admins] spawn {
			...
			_fadeOut = {
				...
				if(!isNull findDisplay 602)then
				{
					...
					if(time > _lastCall)then
					{
						...
						[_name,_puid,'HLOG',toArray(_log)] call "+_AH3458KICKLOG+";
					};
				};
			};
			_dis_head = 0.20;
		...
		"; if(_MOD == 'Epoch')then{ _A3AHstring = _A3AHstring + "
			_playerhandlers = ['InventoryOpened','InventoryClosed','Put','Take','HandleRating'];
		...
		while{1==1}do
		{
						...
						if(_shape isEqualTo 'POLYLINE')then
						{
							...
							if!(_puid isEqualTo '')then
							{
								...
								if(_puid isEqualTo _varuid)then
								{
									...
									if(_need_hlog)then
									{
										[_name,_varuid,'HLOG',toArray(_log)] call "+_AH3458KICKLOG+";
									};
									_need_hlog = false;
								};
							};
			...
			if(_combatTime <= 30)then
			{
				...
				{
					_inCombat = 1;
					[_name,_puid,'COMBATCHECK',_inCombat] call "+_AH3458KICKLOG+";
				};
			}
			else
			{
			...
			if(!isNull _display24)then
			{
				...
				if(_txt != '')then{
					...
					if(_puid in _admins)then
					{
						if(_txt == '!admin')exitWith
						{
							waitUntil {(findDisplay 24) closeDisplay 0;isNull findDisplay 24};
							[_name,_puid,'AC',toArray(_txt)] call "+_AH3458KICKLOG+";
							(findDisplay 46)closeDisplay 0;
						};
					}
					else
					...
					if(_txt in ['killme','/killme','kill me','/kill me','/suicide','suicide'])then{
						waitUntil {(findDisplay 24) closeDisplay 0;isNull findDisplay 24};
						_unit = player;
					...
					"; if(_VDN)then{ _A3AHstring = _A3AHstring + "
						if(_txt in ['!vote day','/vote day','vote day','!day','/day'])then{
							waitUntil {(findDisplay 24) closeDisplay 0;isNull findDisplay 24};
							[_name,_puid,'VOTE',toArray 'DAY'] call "+_AH3458KICKLOG+";
						};
						if(_txt in ['!vote night','/vote night','vote night','!night','/night'])then{
							waitUntil {(findDisplay 24) closeDisplay 0;isNull findDisplay 24};
							[_name,_puid,'VOTE',toArray 'NIGHT'] call "+_AH3458KICKLOG+";
						};
					"; }; _A3AHstring = _A3AHstring + "
				};
			};
		...
		_log = 'Loop #0 ended!';
		[profileName,getPlayerUID player,'BAN',toArray(_log)] call "+_AH3458KICKLOG+";
		[] spawn "+_AHKickOFF+";
	};
	[_name,_puid,_admins] spawn {
		_name = _this select 0;
									...
									if!(_x in _alreadyCheckedVariables)then
									{
										...
										
										{
											if!((toLower _var) find _x isEqualTo -1)then
											{
												...
												[profileName,getPlayerUID player,'HLOG_SKICK',toArray(_log)] call "+_AH3458KICKLOG+";
												[] spawn "+_AHKickOFF+";
												...
												"; if(_UBV2C)then{ _A3AHstring = _A3AHstring + "
													...
													if(!isNil '_m')then
													{
														...
														if((toLower _mstring) find _x != -1)then
														{
															...
															[profileName,getPlayerUID player,'SLOG',toArray(_log)] call "+_AH3458KICKLOG+";
															(findDisplay 46)closeDisplay 0;
															missionNamespace setVariable[_var,nil];
														};
													};
		...
		[_name,_puid] spawn {
			...
			_FiredCheck = {
				...
				if(("+str _UFA+") && {((!("+str _UAW+") && {_ammo in "+str _ForbiddenAmmo+"}) || (("+str _UAW+") && {!(_ammo in "+str _AmmoWhiteList+")}))})then
				{
					...
					[profileName,getPlayerUID _unit,'BAN',toArray(_log)] call "+_AH3458KICKLOG+";
					[] spawn "+_AHKickOFF+";
				};
								...
								if(_maxAmmo > 1)then
								{
									...
									if(diag_tickTime - "+_lastshotmade+" > 0.2)then
									{
										...
										if((_camm >= _maxAmmo)&&(_muzzle isEqualTo currentWeapon _unit))then
										{
											...
											[profileName,getPlayerUID _unit,'SLOG',toArray(_log)] call "+_AH3458KICKLOG+";
				...
				if((!isNull (findDisplay 12))&&(visiblemap))then
				{
					...
					if(!_loggedOnce)then
					{
						...
						if(_size > 5)then
						{
							...
							[_name,_puid,'SLOG',toArray(_log)] call "+_AH3458KICKLOG+";
							_loggedOnce = true;
							lbClear _control2;
						};
					};
						...
						if((isNull _driver)||{(player isEqualTo _driver)})then
						{
							if(!isNil '"+_AHpos+"')then
							{
								...
								if(_tmpAHpos isEqualTo [])then
								{
									...
									if(_tpcount > 3)then
									{
										[_name,_puid,'TMPBAN',toArray(_log)] call "+_AH3458KICKLOG+";
										[] spawn "+_AHKickOFF+";
									}
									else
									{
								...
								else
								{
									...
									[_name,_puid,'TPLOG',toArray(_log),_tmpAHpos select 0,_tmpAHpos select 1,_lastpos,_curpos] call "+_AH3458KICKLOG+";
								};
							}
							else
							{
...
"; }; _A3AHstring = _A3AHstring + "
		...
		[_name,_puid,_admins] spawn {
					...
					[] spawn {
						...
						if(time - "+_clickOnMapTimer+" > 5)then
						{
							...
							if("+_clickOnMapCaught+" >= 3)then
							{
								...
								[profileName,getPlayerUID player,''SLOG_SKICK'',toArray(_log)] call "+_AH3458KICKLOG+";
							};
						};
					};
				';
			...
			while{1==1}do
			{
				...
				if(!isNull player)then
				{
					...
					"; if(_LVC)then{ _A3AHstring = _A3AHstring + "
						...
						if(!(player isEqualTo _veh) && !(_vehptype isEqualTo (typeOf _veh)))then
						{
							...
							{
								[_name,_puid,'LVC',player,_vehptype,mapGridPosition _veh] call "+_AH3458KICKLOG+";
							};
						};
					"; }; _A3AHstring = _A3AHstring + "
				};
									...
									if(!isNull _x)then
									{
										...
										"; if(_MOD == 'Epoch')then{ _A3AHstring = _A3AHstring + "
											...
											if(typeName _getCrypto == 'SCALAR')then
											{
												...
												{
													if(_getCrypto > 250000 || _getCrypto < 0)then
													{
				...
				"; if(_MOD == 'Epoch')then{ _A3AHstring = _A3AHstring + "
					...
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Money Hack: [%1] should be [%2]',life_cash,_rnd1])] call "+_AH3458KICKLOG+";[] spawn "+_AHKickOFF+";
					};
					if!(_rnd2 isEqualTo life_adminlevel)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Adminlevel Hack: [%1] should be [%2]',life_adminlevel,_rnd2])] call "+_AH3458KICKLOG+";[] spawn "+_AHKickOFF+";
					};
					if!(_rnd3 isEqualTo life_coplevel)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Coplevel Hack: [%1] should be [%2]',life_coplevel,_rnd3])] call "+_AH3458KICKLOG+";[] spawn "+_AHKickOFF+";
					};
					if!(_rnd4 isEqualTo life_fnc_MPexec)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Hack: life_fnc_MPexec [%1] should be [%2]',life_fnc_MPexec,_rnd4])] call "+_AH3458KICKLOG+";[] spawn "+_AHKickOFF+";
					};
					if!(_rnd5 isEqualTo life_atmbank)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Hack: life_atmbank [%1] should be [%2]',life_atmbank,_rnd5])] call "+_AH3458KICKLOG+";[] spawn "+_AHKickOFF+";
					};
	...
	_hours = floor(serverTime / 60 / 60);_value = ((serverTime / 60 / 60) - _hours);if(_value == 0)then{_value = 0.0001;};_minutes = round(_value * 60);_seconds = '3458';
	_devLog = format['<infiSTAR.de> %1 VERSION: 28-Sep-2016 21-05-38 infiSTAR.de AHAT (v0241) - server running: %2:%3:%4',VERSIONCHECKRESULT,_hours,_minutes,_seconds];diag_log _devLog;
...
_from = 'magnus@jetprint.se';
_vers = '28-Sep-2016 21-05-38 - v0241';
...
[] spawn {
			...
			{
				{
					...
					{
						_name = name _x;
						[_name,_puid,'BAN',toArray('')] call "+_FNC_AH3458_KICKLOG+";
					};
				} forEach playableUnits;
	...
	"; if((_MOD == 'Epoch')&&(_MPH))then{ _A3AHstring = _A3AHstring + "
		LOG_MPHIT_HANDLER =
		{
			...
			if(_log!='')then
			{
				'ARMA_LOG' callExtension format['A3_MPHIT:%1 (v0241)',_log];
			};
		};
...
_DO_THIS_MORE_OFTEN = {
	...
	if(hasInterFace)then
	{
		...
		} forEach ['"+_AH3458_KICKLOG+"','"+_adminsA+"','"+_MC+"'];
		{
			if(!isNil _x)then
			{
				_var = missionNamespace getVariable _x;
		...
		};
		{
			...
			if(!isNil '_pNs')then
			{
				...
				[profileName,getPlayerUID player,'HLOG',toArray(_log)] call "+_AH3458KICKLOG+";
				profileNamespace setVariable[_x,nil];saveprofileNamespace;
			};
		...
		while{1==1}do
		{
			...
			{
				_timer0 = time + 20;
				'"+_AH3458_KICKLOG+"' addPublicVariableEventHandler {(_this select 1) call "+_FNC_AH3458_KICKLOGSPAWN+";};
				"; if(_MPF)then{ _A3AHstring = _A3AHstring + "
					"; if(_MOD == 'AltisLife')then{ _A3AHstring = _A3AHstring + "
											...
											if!(_uid in "+_tokenSentArray+")then
											{
												if(_uid in "+_connectedArray+")then
												{
													...
													if(_timePassed > 60)then
													{
														...
														[_name,_uid,'HLOG_SKICK',toArray(_log)] call "+_FNC_AH3458_KICKLOG+";
													};
												};
											};
										"; }; _A3AHstring = _A3AHstring + "
...
_zeroCode = {
	if(hasInterface)then
	{
		_AHKickOFF = compileFinal '
			sleep 0.1;
			(findDisplay 46)closeDisplay 0;
			if(!isNil''"+_AH3458KICKLOG+"'')then{[profileName,getPlayerUID player,''KICKME''] call "+_AH3458KICKLOG+";};
		';
		if(isNil'"+_AHKickOFF+"')then{"+_AHKickOFF+" = _AHKickOFF;};
		if(str _AHKickOFF != str "+_AHKickOFF+")then{[] spawn _AHKickOFF;};
		_AH3458KICKLOG = compileFinal ""
			if(!isNil'"+_YourPlayerToken+"')then
			{
				"+_AH3458_KICKLOG+" = ["+_YourPlayerToken+",_this,netId player];publicVariableServer '"+_AH3458_KICKLOG+"';"+_AH3458_KICKLOG+"=nil;
			}
			else
			{
				_this spawn {
					...
					if(isNil'"+_YourPlayerToken+"')exitWith{KICKED_FOR_NIL_TOKEN_AH3458KICKLOG = format['PLAYER__%1__%2',getPlayerUID player,_this];publicVariableServer 'KICKED_FOR_NIL_TOKEN_AH3458KICKLOG';(findDisplay 46)closeDisplay 0;};
					"+_AH3458_KICKLOG+" = ["+_YourPlayerToken+",_this,netId player];publicVariableServer '"+_AH3458_KICKLOG+"';"+_AH3458_KICKLOG+"=nil;
				};
			};
		"";
		if(isNil'"+_AH3458KICKLOG+"')then{"+_AH3458KICKLOG+" = _AH3458KICKLOG;};
		if(str _AH3458KICKLOG != str "+_AH3458KICKLOG+")then{[] spawn _AHKickOFF;};
...
call compile ("
	FNC_DO_CLIENT_VAR_DUMP = compileFinal ""
		...
		{
			'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%1(%2) --- CURRENTLY DUMPING - WAIT UNTIL IT IS DONE (max 120s) --- (v0241)',_name,_puid];
			diag_log format['<infiSTAR.de>A3_CLIENT_VAR_DUMP_%1_%2| %1(%2) --- CURRENTLY DUMPING - WAIT UNTIL IT IS DONE (max 120s) --- (v0241)',_name,_puid];
		};
		...
		{
			'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2: Need at least 1 more connected & alive player to dump vars of %1(%2)! (v0241)',_name,_puid];
			diag_log format['<infiSTAR.de>A3_CLIENT_VAR_DUMP_%1_%2| Need at least 1 more connected & alive player to dump vars of %1(%2)! (v0241)',_name,_puid];
		};
		...
		{
			'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%1(%2) --- ALREADY DUMPING THIS PLAYER --- (v0241)',_name,_puid];
			diag_log format['<infiSTAR.de>A3_CLIENT_VAR_DUMP_%1_%2| %1(%2) --- ALREADY DUMPING THIS PLAYER --- (v0241)',_name,_puid];
		};
		...
		[_name,_puid] spawn {
			...
			if(time > _timeOut)exitWith
			{
				'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%1(%2) --- DUMP TIMED OUT --- (v0241)',_name,_puid];
				diag_log format['<infiSTAR.de>A3_CLIENT_VAR_DUMP_%1_%2| %1(%2) --- DUMP TIMED OUT --- (v0241)',_name,_puid];
			};
			...
			{
				'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%1(%2) --- ALL GOOD - NOTHING TO DUMP --- (v0241)',_name,_puid];
				diag_log format['<infiSTAR.de>A3_CLIENT_VAR_DUMP_%1_%2| %1(%2) --- ALL GOOD - NOTHING TO DUMP --- (v0241)',_name,_puid];
			};
			
			'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%1(%2) --- STARTING DUMP --- (v0241)',_name,_puid];
			diag_log format['<infiSTAR.de>A3_CLIENT_VAR_DUMP_%1_%2| %1(%2) --- STARTING DUMP --- (v0241)',_name,_puid];
			{
			...
			'ARMA_LOG' callExtension format['A3_CLIENT_VAR_DUMP_%1_%2:%1(%2) --- DUMP ENDED --- (v0241)',_name,_puid];
			diag_log format['<infiSTAR.de>A3_CLIENT_VAR_DUMP_%1_%2| %1(%2) --- DUMP ENDED --- (v0241)',_name,_puid];
		};
	"";
");
if(_testserver)then{
